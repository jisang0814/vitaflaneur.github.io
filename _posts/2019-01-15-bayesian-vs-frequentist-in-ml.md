...
- ground truth를 모수 $\mathcal{T}$로, 데이터를 $\mathcal{D}$로 추상화하면 $P(\mathcal{D}\mid\mathcal{T})$로 표현되는 구조를 활용한 통계적 추정을 하는 것이다. 추정 자체에서는 오차를 고려하기 위해 $\mathcal{D}$에 대한 분포를 나타내는 $P(\mathcal{D})$를 활용하기도 하지만, $\mathcal{T}$는 고정되어 있어 $P(\mathcal{T})$를 고려하진 않는다. 
...
-  빈도주의자의 경우 risk function을 결정 이론에 활용한다. 
  - risk function: $$R(\theta^{*}, \delta ) \triangleq \mathbb{E}_{p(\tilde{\mathcal{D}}\mid\theta^{*})} \left[ L(\theta^{*} , \delta(\tilde{\mathcal{D}}))\right] = \int L(\theta^{*} , \delta(\tilde{\mathcal{D}}))p(\tilde{\mathcal{D}}\mid\theta^{*})d\tilde{\mathcal{D}}$$ 
...
-  ERM은 드러난 데이터 만으로 분포를 가정해 추정을 하는 방식이기 때문에 overfitting의 위험이 있고 이를 피하기 위해 regularization 을 해야한다. 
  ...
  - Theoretical upper bound: $P \left(\underset{h \in \mathcal{H}}{\mathop{max}} \lvert R_{emp}(\mathcal{D},h) -R(p_* , h) \rvert>\epsilon \right) \leq 2dim(\mathcal{H})e^{-2N\epsilon^2}$
...
- Model selection의 경우 모든 가능한 모델에 대해서 CV을 해서 구할 수도 있다. 그러나 그보다는 Bayesian model selection을 활용하는 것이 효율적일 수 있다.  즉, $p(m\mid\mathcal{D}) = \frac{p(\mathcal{D}\midm)p(m)}{\sum_{m\in\mathcal{M}}{p(m,\mathcal{D})}}$에서부터 MAP, $\hat{m}=\mathop{argmax} p(m\mid\mathcal{D})$를 계산하는 것이다. 특히, 모델이 uniform prior를 가지고 있다고 가정하면, $\hat{m}=\mathop{argmax} p(m\mid\mathcal{D})=\mathop{argmax} p(\mathcal{D}\midm) =\mathop{argmax}\int p(\mathcal{D}\mid\theta)p(\theta\midm) d\theta $ 와 같이 쓸 수 있다. 이때 사용된 적분을 marginal likelihood라고 부르며, 이 방식으로 model selection을 할 경우 무조건 복잡한 모델이 선호되는 걸 막는 Bayesian's Occam's razor 효과를 갖는다. 
...
- 위 marginal likelihood의 적분은 계산하기 어려운 경우가 많다. 따라서 이를 근사하여 사용하곤 하는데, 가장 잘 알려진 방법은 Bayesian Information Criterion (BIC)을 활용하는 것이다. 
  $$BIC \triangleq \mathop{log}p(\mathcal{D}\mid\hat{\theta}_{MLE}) - \frac{dof(\hat{\theta}_{MLE})}{2} \mathop{log}N \approx \mathop{log}p(\mathcal{D})$$
...
- 또한, full bayesian treatment라고도 불리우는 hierarchical bayesian model (HBM)을 사용할 수도 있다.  HBM은 hyperparameter를 도입함으로써 모델 parameter의 분포를 표현하는 방식으로 나타난다. 다만, HBM은 계산이 어려운 경우가 많다. 때문에 empirical bayesian model (EBM)을 활용해 근사하곤 한다. 
  ...
  와 같이 표현하고,  $\eta^{*} = \mathop{argmax}  p(\eta\mid\mathcal{D})​$를 추정하여, 
  $$\displaystyle p(\theta\mid\mathcal{D})\simeq \frac{p(\mathcal{D}\mid\theta)p(\theta\mid\eta^{*})}{p(\mathcal{D}\mid\eta^{*})} ​$$
  로 나타내면 EM algorithm으로 사후확률이 추정가능하다. 이 과정을 EBM이라 부른다. 
  $\eta^{*}$의 추정에서 균등사전확률을 가정하면, $\eta^{*} = \mathop{argmax}  p(\eta\mid\mathcal{D}) =  \mathop{argmax}  p(\mathcal{D}\mid\eta) $ 가 된다. 따라서 EBM을 type II maximum likelihood라고 부르며, ML에서는 evidence procedure라고 부르기도 한다. 또한, EBM은 prior가 data에 독립적이어야 한다는 원칙을 위배함으로써 계산적인 효용을 얻는 방식이다. 
